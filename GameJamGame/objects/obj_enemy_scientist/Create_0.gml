...
function shoot() {
	...
	if(!shooting && !can_see_player(id)) return
	
	// If on shoot cooldown, do not shoot
	if(shoot_cooldown_timer && !shooting) return shoot_cooldown_timer--
	...
	var _laser = instance_create_layer(x - 1, y - (sprite_height / 2), "Entities", obj_red_bullet);
